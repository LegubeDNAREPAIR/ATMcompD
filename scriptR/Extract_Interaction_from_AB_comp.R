...
for(binSize in c(1000000,500000,100000)){
  ...
  AB_GR <- glue::glue("/home/rochevin/Documents/PROJET_THESE/PAPIER_COLINE_AUDE/data/HiC_D/AB_comp/ABcomp_HiC_D_DIvA_{reskb}.bedGraph") %>% 
    import.bedGraph()%>% mutate(score = ifelse(seqnames =="chr22",score*-1,score))
  ...
  for(OEtype in c("OE")){
    ...
    conditionList <-c("DIvA","OHT","OHTATMi","OHTDNAPKi","OHTPARPi")
    ...
    for(n_percentile in c(5,11,21)){
      AB_values <- AB_GR %>% as_tibble() %>% filter(score != 0) %>% group_by(seqnames) %>% nest() %>% mutate(data = map(data,function(x){
      ...
      data.OE.AB.summarised <- data.OE.AB.cis %>% drop_na() %>%  group_by(percentile_n_1,percentile_n_2,Condition) %>% summarise(score =log2( mean(score)))
      ...
      cc <- data.OE.AB.summarised %>% spread(key=Condition,value=score) %>% gather(key = Condition,value = score,-percentile_n_1:-percentile_n_2,-CTRL) %>% 
      ...
      pdf(glue::glue("/media/HDD_ROCHER/PROJET_THESE/PAPIER_COLINE_AUDE/results/heatmap_HiC_D/SADDLE_PLOTS/Heatmap_saddle_{reskb}_percentile_{n_percentile}_{OEtype}.pdf"),width=5,height=27)
      # print(p1)
      print(p1.summarised)
      print(p3.summarised)
      # print(p2.summarised)
