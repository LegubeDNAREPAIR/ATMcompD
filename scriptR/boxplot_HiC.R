...
# cc <- res.boxplot %>% group_by(Var1,Var2,Type,mylim,win,Comparison) %>% summarise(ratio = mean(ratio,na.rm=T),log2ratio = mean(log2ratio,na.rm=T),log10ratio = mean(log10ratio,na.rm=T))
cc <- res.boxplot %>% group_by(Var1,Var2,Type,mylim,win,Comparison)  %>% summarise(DIvA=mean(DIvA,na.rm=T),OHT=mean(OHT,na.rm=T)) %>% mutate(ratio = OHT/DIvA) %>%
  mutate(log2ratio = log2(ratio))
p1 <- cc %>% filter(Comparison != "AdjacentTAD_cis") %>% 
