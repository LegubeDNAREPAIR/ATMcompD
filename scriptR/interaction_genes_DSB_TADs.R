...
res.DSB <- lapply(seqlevels(bless80),function(chrom){
  ...
  diva.chr <- bless80 %>% filter(seqnames ==chrom) %>% 
    anchor_center() %>% mutate(width = 5000) 
...
pdf("/mnt/NAS/DOSSIERS_PERSO/VINCENT/HiC_Coline/Interaction_between_genes_and_DSB_inside_TADs.pdf",height=8,width=6)
print(p1)
print(p1.5)
dev.off()
